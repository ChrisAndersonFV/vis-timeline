 ...
 */
  ...
  focus(id, options) {
    ...
    if (start !== null && end !== null) {
      ...
      // TODO: Is there a better way to provide "offset" option? This does work and at
      // least is simple... but it's not pixel-perfect. Maybe that's fine?
      const rangeStart = middle - interval / 2;
      const rangeEnd = middle + interval / 2;
      const rangeOffset = options && options.offset !== undefined ? options.offset * interval : 0;

      this.range.setRange(rangeStart + rangeOffset, rangeEnd + rangeOffset, { animation }, finalVerticalCallback, verticalAnimationFrame);
    }
  }
